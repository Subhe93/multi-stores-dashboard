...
export default function CreatorCategoriesPage() {
  ...
  // Client-side filter over the visible rows — the tree endpoint has no search,
  // and collections lists are small enough to filter in memory.
  const visibleRows = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return rows;
    return rows.filter(
      (row) =>
        row.display_path.toLowerCase().includes(q) ||
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteError(e?.message || t('collections.failedDelete'));
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        emptyMessage={rows.length > 0 ? t('collections.noSearchResults') : ''}
      />
