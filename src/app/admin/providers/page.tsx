  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage product providers and verify applications</p>
      </div>

      <DataTable
        columns={[
          { key: 'company', label: 'Company', render: (item: Provider) => (
            <div>
              <p className="text-sm font-medium">{item.company_name}</p>
              <p className="text-[10px] text-muted-foreground">{item.user.email}</p>
            </div>
          )},
          { key: 'country', label: 'Country', render: (item: Provider) => (
            <span className="text-sm">{item.country}</span>
          )},
          { key: 'phone', label: 'Phone', render: (item: Provider) => (
            <span className="text-xs text-muted-foreground">{item.phone || '—'}</span>
          )},
          { key: 'verified', label: 'Status', render: (item: Provider) => (
            item.verified
          ...
          { key: 'created_at', label: 'Joined', render: (item: Provider) => (
          ...
          { key: 'actions', label: '', render: (item: Provider) => (
            <div className="flex gap-1">
              ...
              <Button variant="ghost" size="sm" className="h-6 text-[10px]" onClick={() => router.push(`/admin/providers/${item.id}`)}>View</Button>
            </div>
          )},
        ]}
        data={providers}
        ...
        emptyMessage={loading ? 'Loading...' : 'No providers found'}
      />
    </div>
  );
}
